...
def same_digits(a, b):
    ...
    assert a > 0 and b > 0
    while a and b:
        if a % 10 == b % 10:# if a != b:, it is to certify every digit continuously in both a and b
            end = a % 10
            while end == a%10: #so after both of them equivalent, then compare the prev and next in itself
                a = a // 10
            while end == b % 10:
                b = b // 10
        else:
