...
def sequence(n, term):
    ...
    """
    t, k = 0, 1
    while k <= n:#focus on k and k is up to n graduated by 1 
         m = 1
         x = term(k)#  x = n-1 # while means execute something in a range of something, so since k raise repeatedly, we should let term to apply
         while m <= x:
             m *= 10 #extend the digits
         t = x + m * t # m * t means multiply the result to the left side digits and there be other result following by the right, x is the number on single digit
         k = k + 1
    return t 
