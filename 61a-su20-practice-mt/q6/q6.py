...
def make_zipper(f1, f2, sequence):
    ...
    100
    """
    #Make it clear !!! the order of execution is from inside to outside, and the the value transfered by the assignment should be execute that is why there need a HIGH ORDER lambda in helper  
    zipper = lambda m:m
    helper = lambda x, y: lambda m : x(y(m))
    while sequence > 0:
        if sequence % 10 == 1:
