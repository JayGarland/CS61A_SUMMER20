...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        # BEGIN PROBLEM 4
        "*** YOUR CODE HERE ***"
        procedure = scheme_eval(first, env) #get the operator
        validate_procedure(procedure)
        args = rest.map(lambda operand : scheme_eval(operand, env)) #to apply every operand in pairs by using lambda
...
def eval_all(expressions, env):
    ...
    """
    # BEGIN PROBLEM 7
    # "begin" as is (begin <expression>) Evaluates each expression in order in the current environment, returning the evaluated last one
    # if expressions == nil:
    #     return nil
          # change this line
    # else:
...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError('Incorrect number of arguments to function call')#using raise when there should be error
        frame = Frame(self)# get assigned that store defines in the frame
        while (formals and vals) != nil:
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 5
        "*** YOUR CODE HERE ***"
        env.define(target, scheme_eval(expressions.rest.first, env)) #take it as the another form of higher order function, while the expression.rest.first would be the first parameter in the next call,
        return target
        # END PROBLEM 5
        ...
        "*** YOUR CODE HERE ***"
        name = target.first
        lambdaformals = target.rest
        body = Pair(lambdaformals, expressions.rest) #since do_lambda_form needs to construct a lambda expression by using a legal formal parameter expression so that we should pair in a way to meet it 
        #then find way to bind LambdaProcedure, remember env has a method of define, which is the instance in Frame class of object that has a attribute bindings, thus we can bind objects together through this way
        env.define(name, do_lambda_form(body, env))
        return name
        # END PROBLEM 9
    else:
...
def do_and_form(expressions, env):
    ...
    while expressions.rest != nil:
        neweval = scheme_eval(expressions.first, env)
        if is_false_primitive(neweval):
            return False
...
def make_let_frame(bindings, env):
    ...
    "*** YOUR CODE HERE ***"
    while bindings != nil:
        binding = bindings.first # we only mutate the elements in the bindings first  
        validate_form(binding, 2, 2) # to check the binding meets the condition

        names = Pair(binding.first, names) #in loop, we add names values by reassigning it to another value, and store the previous one by the Pair function
        values = Pair(scheme_eval(binding.rest.first, env), values) # using pair to construct another scheme list by the first arg is a value then following a scheme list next
...
class MuProcedure(Procedure):
    ...
    # BEGIN PROBLEM 15
    "*** YOUR CODE HERE ***" 
    # another type of scoping, which is not standard in scheme, is called dynamic scoping: the parent of the frame is the environment in which the procedure was evaluated. with dynamic scoping, calling the same procedure in different parts of your code  can lead to different results
    # because of the F get evaluated within the procedure g, it has access to the a and b defined in g's frame
    # but evaluates to a muprocedure instance that is dynamically scoped. most of the muprocedure
    # when a muprocedure created by a mu expressionis called the parent of the new call frame is the environment in which the call expression was evaluated 
    # muprocedure does not need to store an environment as an instance attribute. it can refer to names in the environment from which it was called
    def make_call_frame(self, args, env):
        while args != nil:
            Frame.define(env, self.formals.first, args.first) #to assign value to a formal
            self.formals = self.formals.rest
            args = args.rest
        return env #is the same to lamdbaprocedure that return a moddified Frame
    # END PROBLEM 15
