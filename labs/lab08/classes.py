...
class TACard(Card):
    ...
    def effect(self, other_card, player, opponent):
        ...
        """
        "*** YOUR CODE HERE ***"
        swapper = other_card.attack
        other_card.attack = other_card.defense
        other_card.defense = swapper
...
class ProfessorCard(Card):
    ...
    def effect(self, other_card, player, opponent):
        ...
        for everyone in player.deck.cards:
                everyone.attack +=other_card.attack
                everyone.defense +=other_card.defense
        for anyone in list(opponent.deck.cards):
                if other_card.attack == anyone.attack or other_card.defense == anyone.defense:
                    opponent.deck.cards.remove(anyone)
        ...
        if discarded:
            ...
            return
            #Uncomment the line below when you've finished implementing this method!
            # nextone = iter(player.hand)
            # nextoppo = iter(opponent.hand)
            # while player.draw:
            #     next(nextone).attack += other_card.attack
            #     next(nextone).defense += other_card.defense
            #     while opponent.draw:
            #         if other_card.attack == next(nextoppo).attackor or other_card.defense== next(nextoppo).defense:
            #              opponent.hand.pop(4)
            #              opponent.draw()
    def copy(self):
