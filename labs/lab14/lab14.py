...
def align_skeleton(skeleton, code):
    ...
    def helper_align(skeleton_idx, code_idx):
        ...
        if skel_char == code_char:
            edits, cost = helper_align(skeleton_idx+ 1, code_idx+ 1) #get values from what the helper returns
        ...
        possibilities.append((prefix + edits, cost + 1))
        return min(possibilities, key=lambda x: x[1])#switch the position of code and skeleton
...
def num_splits(s, d):
    ...
    def helper(first, second, index): #two division approaching, and a identifier
        nonlocal cnt #recorder
        if index == length:# end of process
            if(abs(first - second) <= d): # as following in command of introduction
                cnt += 1
            return #stop
        else:
            helper(first + s[index], second, index +1)
            helper(first, second + s[index], index + 1)
            #both way pushing forward index till to the length
    helper(0, 0, 0)#init
    return cnt//2 #since pushed twice so fix the real count
...
def insert(link, value, index):
    ...
    """
    if index == 0:
        tmp = Link(link.first, link.rest) #store the previous status
        link.first = value #change it for base case
        link.rest = tmp #restore the previous within current link
